...
geotab.addin.addinMediaFiles = function () {
    ...
    const resizeImage = (mediaFile, width) => {
        return new Promise((resolve, reject) => {
            ...
            if (contentType.startsWith('video') || contentType.endsWith('webp') || contentType.endsWith('gif')|| contentType.endsWith('pdf')) {
                ...
                methods: {
                    list() {
                        return new Promise((resolve, reject) => {
                           ...
                           addinVue.getCurrentUserProfile().then(userLoggedIn =>{
                        ...
                        if(userLoggedIn.isDriver)
                        {
                            addinVue.isDriver = true;
                            paramsReq={
                                typeName: 'MediaFile',
                                resultsLimit: 100, 
                                search:{
                                    'driverSearch':{ 'id':userLoggedIn.id}
                                },
                            };
                        }
                            /*opt nomin*/
                            api.call('Get',paramsReq, result => {
                                result.sort((a, b) => {
                    ...
                    getCurrentUserProfile(){
                        ...
                        return new Promise((resolve, reject) => {
                        if(addinVue.credentials.username == null){
                            api.getSession(cr => {
                                const getUrl = s => {
                                    if (s.startsWith('http')) {
                                        return new URL(s).hostname;
                        ...
                        api.call('Get', {
                            ...
                            resultsLimit: 10,
                            search:{
                                name:addinVue.credentials.userName
                            },
                        }, result => {
                            resolve(result[0]);
                            return;
                    ...
                    getDevicesList(userLoggedIn){
                        
                            api.call('Get', {
                                typeName: 'Device',
                                'search': {
                                    'groups': userLoggedIn.companyGroups
                                }
                            }, function(result) { 
                                console.log(result);
                            })
